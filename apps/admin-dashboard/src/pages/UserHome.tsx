...
import { ChevronRight, Layout, Maximize2, FileText, Home, MapPin } from 'lucide-react';
import GPSInstructionModal from '../components/GPSInstructionModal';

const UserHome = () => {
  const navigate = useNavigate();
  const [gpsStatus, setGpsStatus] = useState<'requesting' | 'active' | 'error' | 'permission-denied' | 'position-unavailable' | 'insecure-context'>('requesting');
  ...
  useEffect(() => {
    ...
    const startTracking = (highAccuracy = true) => {
      ...
      const isSecure = window.isSecureContext || 
                      ...
                      window.location.hostname === 'score.golfinity.id';

      if (!isSecure && window.location.protocol !== 'https:') {
        setGpsStatus('insecure-context');
        return;
      }
      ...
      if ("geolocation" in navigator) {
        watchId = navigator.geolocation.watchPosition(
          (position) => {
            setGpsStatus('active');
            const tabletCode = localStorage.getItem('golf_tablet_code') || 'Unit-01';
            const gpsData = {
          ...
          (err) => {
            ...
            if (err.code === 1) { // PERMISSION_DENIED
              ...
              // Try fallback to low accuracy if high fails
              if (highAccuracy) {
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#1a1a1a] overflow-hidden font-sans text-white select-none flex flex-col">
        ...
        <div className={`flex items-center gap-2 px-4 py-2 rounded-full backdrop-blur-md border ${
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.2em]">
            {gpsStatus === 'active' ? 'GPS Active & Monitoring' : 
             ...
             gpsStatus === 'insecure-context' ? 'HTTPS Required for GPS' :
             gpsStatus === 'error' ? 'GPS Error - Check Hardware' : 'Requesting GPS...'}
          </span>
        </div>
        
        {(gpsStatus === 'permission-denied' || gpsStatus === 'position-unavailable' || gpsStatus === 'insecure-context') && (
          <button 
            onClick={() => {
              if (gpsStatus === 'insecure-context') {
                alert("GPS tidak bisa aktif karena koneksi tidak aman (HTTP). Gunakan HTTPS atau akses via localhost.");
              } else if (gpsStatus === 'permission-denied') {
          ...
          >
            {gpsStatus === 'insecure-context' ? 'Bantuan Koneksi Aman' : 
      ...
      <div 
        ...
        style={{ 
          backgroundImage: `url('/images/palmspring-karawang.jpg')`,
      ...
      {/* Top Left Logo (Palms Springs Placeholder) */}
      <div className="absolute top-6 left-6 md:top-8 md:left-8 flex items-center gap-2 z-10">
        <div className="w-10 h-10 md:w-12 md:h-12 bg-white/10 backdrop-blur-md rounded-lg flex items-center justify-center border border-white/20">
          <img 
            src="https://www.palmspringsgolf.com/wp-content/uploads/2019/04/cropped-palm-springs-logo-1.png" 
            alt="Logo" 
            className="w-8 h-8 md:w-10 md:h-10 object-contain invert brightness-200"
            onError={(e) => {
              (e.target as HTMLImageElement).style.display = 'none';
            }}
          />
        </div>
        <div className="text-[8px] md:text-[10px] font-bold tracking-widest text-white/80">
          PALMSPRINGS
          <div className="text-[6px] md:text-[8px] font-normal tracking-normal opacity-60">GOLF & COUNTRY CLUB</div>
        </div>
      </div>
      ...
      <div className="relative flex-1 flex items-center justify-center md:justify-end px-6 sm:px-16 md:px-32">
        <div className="max-w-md w-full space-y-6 md:space-y-8 text-center md:text-left">
          ...
          <div className="space-y-3 md:space-y-4 pt-2 md:pt-4">
            {[
              { name: "Hole Yardage & Layout", path: "hole-yardage" },
              { name: "Club Panorama View", path: null },
              { name: "Club Summary", path: "club-summary" }
            ].map((feature, index) => (
              <div 
                key={index} 
          ...
          <div className="pt-6 md:pt-8 flex justify-center md:justify-start">
            <button 
              ...
              className="group relative bg-blue-600 hover:bg-blue-500 px-8 py-3.5 md:px-10 md:py-4 rounded-full flex items-center gap-3 md:gap-4 transition-all active:scale-95 shadow-2xl shadow-blue-600/30"
            >
              <span className="text-lg md:text-xl font-bold tracking-wider">Round Start</span>
